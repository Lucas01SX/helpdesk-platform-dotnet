...
public sealed class EmailVerificationTokenTests
{
    ...
    {
        var token = EmailVerificationToken.Create(Guid.NewGuid(), "hash", Now);

        token.IsValid(Now).Should().BeTrue();
    }

    [Fact]
    public void Should_Expire_After_24_Hours()
    {
        var token = EmailVerificationToken.Create(Guid.NewGuid(), "hash", Now);
        var afterExpiry = Now.AddHours(24).AddSeconds(1);
    ...
    [Fact]
    public void Should_Set_Expiry_At_24_Hours_From_Creation()
    {
        var token = EmailVerificationToken.Create(Guid.NewGuid(), "hash", Now);
